...
public class StargateFileReader {
	public Stargate getStargate(String name) {
		if (name != null) {
			try {
				...
				e.printStackTrace();
				System.out.println("Konnte stargateList.txt nicht finden!");
			}
		}
	}

	public boolean saveStargate(Stargate stargate) {
		if (getStargate(stargate.name) != null) {
			return false;
		}
	...
	public boolean deleteStargate(Stargate stargate) throws IOException {
		...
		while ((currentLine = reader.readLine()) != null) {
			String currentLine;
			if ((!currentLine.contains(stargate.name)) && (!currentLine.startsWith("#"))) {
	...
	public String StargateToString(Stargate s) {
		String ret = s.name + ";" + s.worldID + ";" + s.shieldStatus + ";" + s.activationStatus + ";" + s.loc + ";"
	...
	public Stargate StringToStargate(String string) {
		...
		s.name = str[0];
		s.worldID = Integer.parseInt(str[1]);
		s.shieldStatus = Boolean.parseBoolean(str[2]);
		s.activationStatus = Boolean.parseBoolean(str[3]);
		...
		s.loc = new org.bukkit.Location((org.bukkit.World) Bukkit.getWorlds().get(s.worldID), locX, locY, locZ);
			...
			try {
				...
				try {
					...
					while ((sCurrentLine = br.readLine()) != null) {
						...
						if (!sCurrentLine.startsWith("#")) {
							Stargate stargate = StringToStargate(sCurrentLine);
							if ((stargate.activationStatus) && (!stargate.target.equals("null"))) {
								GateList.add(stargate);
							}
						}
					}
