...
public class Kawoosh {
		...
		if (Main.getInstance() != null) {
			...
			this.id = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(Main.getInstance(), new Runnable() {
				public void run() {
					...
					ArrayList<Vector> kawooshCoords = new ArrayList();
					...
					for (Vector v : kawooshCoords) {
						...
						Location location = new Location(
								(org.bukkit.World) Bukkit.getWorlds().get(Kawoosh.this.stargate.worldID), k.getX(),
								k.getZ(), k.getY());
	...
	public ArrayList<Vector> state1() {
		ArrayList<Vector> coords = new ArrayList();
	...
	public void cleanKawoosh() {
     ...
     int i; for (Iterator localIterator = vecs.iterator(); localIterator.hasNext(); 
         i < 8)
     {
       ...
       Location location = new Location(
         (org.bukkit.World)Bukkit.getWorlds().get(this.stargate.worldID), k.getX(), k.getZ(), k.getY());
       Block block = location.getBlock();
       ...
       if (!block.hasMetadata("StargateBlock"))
       {
         ...
         if (block.hasMetadata("PortalWater")) {
           block.removeMetadata("PortalWater", Main.getInstance());
         }
         if (block.hasMetadata("Kawoosh")) {
           block.removeMetadata("Kawoosh", Main.getInstance());
         }
       }
       i++;
     }
